...
Projectile = Class(QCEProjectile) {
    OnImpact = function(self, targetType, targetEntity)
        ...
        if DD.DamageAmount == nil then return end
        ...
        if targetType == 'Shield' then
            local shieldDD = LOUDCOPY(self.DamageData)
            if STRINGSUB(DamageType, 1, 10) == 'ShieldMult' then
                local mult = TONUMBER(STRINGSUB(DamageType, 11)) or 1
                shieldDD.DamageAmount = DD.DamageAmount * mult
            end
            self.DamageData = shieldDD
        end
